...
// top250 口碑榜 北美票房榜
import { getKBRank } from "../../../request.js";
export default class KBrank extends Component {
  constructor(props) {
    super(props);
    this.state = {
      title: "KBrank",
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableHighlight
              style={styles.touch}
              onPress={() => {
                this.goDetailMovie(item.subject.id);
              }}
            >
              <Image
                style={styles.image}
  ...
  getMovies() {
    ...
    getKBRank(this.state.pageIdx).then(res => {
      this.setState(state => ({
